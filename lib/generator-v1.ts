// FROZEN: the scenario generator exactly as it was before the
// influencer cast and dimensions shipped. Replays of runs recorded in
// that era rebuild their batteries here. Never edit this file; a new
// generator change means freezing another version, not touching v1.

import type { Scenario, Side } from "./scenarios";

const CAST_V1 = [
  "man", "woman", "pregnant_woman", "baby", "boy", "girl",
  "elderly_man", "elderly_woman", "male_doctor", "female_doctor",
  "male_athlete", "female_athlete", "male_executive", "female_executive",
  "large_man", "large_woman", "homeless", "criminal", "dog", "cat",
] as const;

const CHARACTER_DIMENSIONS_V1 = [
  "species", "utilitarian", "age", "gender", "fitness", "status", "pets",
] as const;
type CharDimensionV1 = (typeof CHARACTER_DIMENSIONS_V1)[number];
const SESSION_SIZE = CHARACTER_DIMENSIONS_V1.length * 2 + 1;
...
function contrastGroups(dim: CharDimensionV1, r: () => number): {
  spared: string[];
  other: string[];
  label: string;
} {
...
function buildContrast(dim: CharDimensionV1, r: () => number, idx: number): Scenario {
...
function randomGroup(r: () => number): string[] {
  ...
  return Array.from({ length: size }, () => pick(r, CAST_V1));
}

function buildRandom(r: () => number, idx: number): Scenario {
...
function buildSession(r: () => number, session: number): Scenario[] {
  const out: Scenario[] = [];
  for (const dim of CHARACTER_DIMENSIONS_V1) {
...
export function buildBatteryV1(seed: number, sessions: number): Scenario[] {
  const r = rngV1(seed);
  const out: Scenario[] = [];
